...
const TaskModal = ({ isOpen, onClose, onSave, editingTask = null }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center sm:items-end z-[1000] p-4 sm:p-0 animate-fadeIn" role="dialog" aria-modal="true">
            <div className="w-full max-w-4xl sm:max-w-full bg-white sm:bg-white rounded-3xl sm:rounded-t-3xl sm:rounded-b-none shadow-2xl flex flex-col max-h-[95vh] sm:max-h-[92vh] animate-slideUp">
                <div className="p-5 px-8 sm:px-5 sm:py-4 border-b border-[#f1f5f9] flex justify-between items-center bg-slate-50/50 shrink-0 rounded-t-3xl">
                    {/* Mobile drag handle */}
                    <div className="hidden sm:block absolute top-2 left-1/2 -translate-x-1/2 w-10 h-1 bg-slate-200 rounded-full" />
                    <h2 className="text-xl sm:text-base font-bold text-[#1a202c]">{editingTask ? 'Edit Task' : 'Create New Task'}</h2>
                ...
                <form onSubmit={handleSubmit} className="p-8 sm:p-4 md:p-6 overflow-y-auto flex-1">
                    <div className="grid grid-cols-4 gap-5 sm:gap-3 md:grid-cols-1 sm:grid-cols-1">
                        <div className="flex flex-col gap-1.5 col-span-4 sm:col-span-1">
                        ...
                        <div className="flex flex-col gap-1.5 col-span-2 md:col-span-1 sm:col-span-1 relative" ref={dropdownRef}>
                        ...
                        <div className="flex flex-col gap-1.5 col-span-2 md:col-span-1 sm:col-span-1 relative" ref={leadDropdownRef}>
                        ...
                        {/* On mobile: Phone + Email side by side */}
                        <div className="flex flex-col gap-1.5 col-span-1 sm:col-span-1 sm:[grid-column:1]">
                        ...
                        <div className="flex flex-col gap-1.5 col-span-2 md:col-span-1 sm:col-span-1">
                    ...
                    {!editingTask && (
                        <div className="flex items-center gap-4 sm:gap-3 mt-5 p-4 sm:p-3 bg-[#f8faff] rounded-2xl border border-[#ebf0ff] cursor-pointer hover:bg-[#f0f4ff] transition-colors" onClick={() => setAddToOutlook(!addToOutlook)}>
                            ...
                            <div className="flex flex-col">
                                <span className="text-[13px] sm:text-[12px] font-black text-[#1a202c]">Sync with Outlook Calendar</span>
                                <span className="text-[11px] sm:text-[10px] font-bold text-[#718096] uppercase tracking-wide sm:normal-case sm:tracking-normal">Automatically add this to your Microsoft 365 schedule</span>
                            </div>
                        </div>
                    )}

                    <div className="flex justify-end gap-3 mt-6 sm:mt-4 pt-4 border-t border-slate-100">
                        <button type="button" className="p-[10px_24px] sm:px-4 rounded-xl text-sm font-bold text-[#718096] hover:bg-[#f8fafc] transition-all" onClick={onClose}>Discard</button>
                        <button type="submit" disabled={isSyncingOutlook} className="bg-[#2447d7] text-white p-[10px_36px] sm:px-6 rounded-xl text-sm font-black shadow-lg shadow-[#2447d7]/25 hover:bg-[#1732a3] hover:translate-y-[-2px] active:translate-y-0 transition-all disabled:opacity-70 flex items-center gap-2 uppercase tracking-wider">
