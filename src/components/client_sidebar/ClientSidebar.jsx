                ...
                {NAV_GROUPS.map((group) => (
                    <div className="mb-6" key={group.group}>
                        ...
                        {group.items.map((item) => (
                            <button
                                key={item.id}
                                className={`w-[calc(100%-32px)] ml-4 h-12 px-4 mb-1 flex items-center gap-3 bg-transparent border-none rounded-[10px] text-[#4a5568] cursor-pointer transition-all duration-200 text-sm font-semibold hover:bg-[#f8fafc] hover:text-[#2447d7] ${activePage.startsWith(item.path) ? 'bg-[#ebf0ff] text-[#2447d7]' : ''}`}
