                    ...
                    {options.map(opt => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full text-left px-4 py-2.5 text-[10px] font-black uppercase tracking-widest transition-all ${isActive ? 'bg-[#2447d7] text-white' : (isDark ? 'text-slate-300 hover:bg-white/5' : 'text-slate-600 hover:bg-slate-50')}`}
                            >
                                {opt}
                            </button>
                        );
                        ...
                        >
                            ...
                            <div className="flex flex-col min-w-0">
                                <span className={`text-[11px] font-extrabold uppercase leading-none mb-1 transition-colors ${isDark ? 'text-[#e4ecff]' : 'text-[#1e293b]'} group-hover/names:text-[#2447d7]`}>
                                    {showAllNames ? info.fullNames.join(', ') : (info.summary === 'Self' ? (user.first_name + ' ' + (user.last_name || '')) : info.summary)}
                                </span>
...
const TeamLeaderCalendar = ({ tasks: initialTasks, setTasks, initialDate, notifyReminderSet }) => {
    ...
    const [calendarMode, setCalendarMode] = useState('Local'); 
    ...
    const filteredTasks = tasks.filter(task => {
        ...
        if (roleFilter !== 'All') {
            ...
            matchesRole = assignedIds.some(id => {
                if (id === 'Self') return roleFilter.toLowerCase() === 'team leader';
                ...
                <div className="flex flex-col gap-4 overflow-hidden h-full">
                    <div className={`p-2 rounded-2xl border flex items-center justify-between gap-3 shrink-0 ${isDark ? 'bg-[#1e2347] border-[#2c3568]' : 'bg-white border-[#edf2f7] shadow-sm'}`}>
                        ...
                        <div className="flex p-0.5 rounded-xl border bg-[#f1f5f9] dark:bg-[#2a3258] border-[#e2e8f0] dark:border-[#36407a] shrink-0">
                            {['All', 'Team Leader', 'Accounts Manager', 'Tele Agent'].map(role => {
