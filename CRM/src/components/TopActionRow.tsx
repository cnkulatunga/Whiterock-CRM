...
type Doc = { id: string | number; title: string; category: string; fileType: string; fileSize: string; version: string; uploaded: string; desc: string };
...
const FILE_ICONS: Record<string, { icon: string; bg: string; color: string }> = {
    ...
    default: { icon: 'fa-solid fa-file',         bg: '#f1f5f9', color: '#475569' },
};

const CAT_COLORS: Record<string, string> = {
    'Knowledge Base': '#1d4ed8',
    'Guides':         '#7e22ce',
    'FAQs':           '#b45309',
    'Products':       '#065f46',
    'Policies':       '#b91c1c',
...
function normalizeDoc(d: any): Doc {
    const ext = (d.file_type || d.fileType || d.filename?.split('.').pop() || 'pdf').toLowerCase();
    return {
        id:       d.id,
        title:    d.title || '',
        category: d.category || '',
        ...
        version:  d.version || '',
        uploaded: (d.created_at || d.uploaded || '').slice(0, 10),
...
export default function TopActionRow({ designMode = false, onToggleDesignMode }: TopActionRowProps) {
    ...
    const TASK_ICON: Record<string, string> = {
        'Priority Call': 'fa-phone',
        'Case Research': 'fa-magnifying-glass',
        'Critical Document': 'fa-file-circle-exclamation',
        'Call': 'fa-phone',
        'Meeting': 'fa-users',
        'Follow-up': 'fa-rotate-right',
        'Email': 'fa-envelope',
        'Document': 'fa-file',
        'Research': 'fa-chart-bar',
    ...
    useEffect(() => {
        ...
        async function loadNotifs() {
            try {
                ...
                const [notifRes, taskRes] = await Promise.allSettled([
                ...
                if (notifRes.status === 'fulfilled' && notifRes.value.ok) {
                    const raw = await notifRes.value.json();
                    const list = Array.isArray(raw) ? raw : (raw.results ?? []);
                    for (const n of list) {
                        merged.push({
                            id: n.id,
                            title: n.title || n.message?.slice(0, 60) || 'Notification',
                            desc: n.description || n.message || '',
                            time: relativeTime(n.created_at || n.timestamp || ''),
                            unread: n.is_read === false || n.unread === true,
                            icon: n.icon || 'fa-bell',
                            color: n.color || 'bg-indigo-50 text-indigo-600',
                            source: 'backend',
                        });
                    }
                }
                ...
                if (taskRes.status === 'fulfilled' && taskRes.value.ok) {
                    const raw = await taskRes.value.json();
                    const list = Array.isArray(raw) ? raw : (raw.results ?? []);
                    for (const t of list) {
                        const s = (t.status || '').toLowerCase();
                        if (s === 'completed' || s === 'done') continue;
                        const taskType = t.type || t.task_type || '';
                        const icon = TASK_ICON[taskType] || 'fa-list-check';
                        const isOverdue = t.date && new Date(t.date) < new Date();
                        merged.push({
                            id: `task-${t.id}`,
                            title: t.title || 'Task assigned',
                            desc: `${taskType ? taskType + ' · ' : ''}${t.client || t.assignee || ''}${isOverdue ? ' · Overdue' : ''}`.replace(/^·\s*|·\s*$/g, '').trim(),
                            time: t.date ? relativeTime(t.date + (t.time ? 'T' + t.time : '')) : '',
                            unread: s === 'pending' || s === 'in progress' || s === 'in_progress',
                            icon,
                            color: isOverdue ? 'bg-rose-50 text-rose-600' : s === 'in progress' || s === 'in_progress' ? 'bg-amber-50 text-amber-600' : 'bg-indigo-50 text-indigo-600',
                            source: 'task',
                        });
                    }
                }
    ...
    useEffect(() => {
        try {
            ...
            fetch('/api/stats', { headers: token ? { Authorization: `Bearer ${token}` } : {} })
                ...
                .then(data => setTotalLeads(data.totalLeads ?? null))
                .catch(() => {});
        } catch { /* ignore */ }
    }, []);
    ...
    const markAllRead = () => {
        const unread = notifList.filter(n => n.unread && n.source === 'backend');
        ...
        try {
            ...
            for (const n of unread) {
                fetch('/api/notifications', { method: 'POST', headers, body: JSON.stringify({ id: n.id }) }).catch(() => {});
            }
        } catch { /* ignore */ }
    };

    const markOneRead = (item: NotifItem) => {
        setNotifList(prev => prev.map(n => n.id === item.id ? { ...n, unread: false } : n));
        if (item.source === 'backend') {
            try {
                const session = sessionStorage.getItem('crm_session');
                const token = session ? JSON.parse(session).access : null;
                const headers: Record<string, string> = { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) };
                fetch('/api/notifications', { method: 'POST', headers, body: JSON.stringify({ id: item.id }) }).catch(() => {});
            } catch { /* ignore */ }
        }
    };
    ...
    return (
        <>
            ...
            <div className="flex items-center gap-2 px-6 pt-4 w-full shrink-0 min-h-[60px]">
                <div className="flex items-center gap-2 flex-nowrap flex-1 min-w-0 overflow-x-auto pb-2 custom-scrollbar">
                    <a href="/leads" className="glass-card p-2.5 flex items-center gap-3 hover:border-indigo-500/50 group transition-all min-w-[140px] cursor-pointer">
                        ...
                        <div className="truncate"><h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Leads</h4><p className="text-[10px] font-black text-slate-900 mt-1">{totalLeads === null ? '—' : totalLeads.toLocaleString()}</p></div>
                    </a>
                    {hasFeature('teams') && (
                        <a href="https://teams.microsoft.com" target="_blank" rel="noreferrer" className="glass-card p-2.5 flex items-center gap-3 hover:border-indigo-400/50 group transition-all min-w-[130px] cursor-pointer">
                            ...
                            <div className="truncate"><h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Connect</h4><p className="text-[10px] font-black text-slate-900 mt-1">TEAMS</p></div>
                        </a>
                    )}
                    {hasFeature('whatsapp_direct') && (
                        <a href="https://web.whatsapp.com" target="_blank" rel="noreferrer" className="glass-card p-2.5 flex items-center gap-3 hover:border-emerald-400/50 group transition-all min-w-[130px] cursor-pointer">
                            ...
                            <div className="truncate"><h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Chat</h4><p className="text-[10px] font-black text-slate-900 mt-1">WHATSAPP</p></div>
                        </a>
                    )}
                    {hasFeature('mail') && (
                        <a href="mailto:admin@whiterock.com" className="glass-card p-2.5 flex items-center gap-3 hover:border-blue-400/50 group transition-all min-w-[130px] cursor-pointer">
                            ...
                            <div className="truncate"><h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Official</h4><p className="text-[10px] font-black text-slate-900 mt-1">MAIL HUB</p></div>
                        </a>
                    )}
                    {hasFeature('premium_calculator') && (
                        <div onClick={() => { setShowCalc(true); setShowDocs(false); }} className="glass-card p-2.5 flex items-center gap-3 hover:border-amber-400/50 group transition-all min-w-[130px] cursor-pointer">
                            ...
                            <div className="truncate"><h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Utility</h4><p className="text-[10px] font-black text-slate-900 mt-1">CALCULATOR</p></div>
                        </div>
                    )}
                    <div onClick={onToggleDesignMode} className={`glass-card p-2.5 flex items-center gap-3 hover:border-indigo-400/50 group transition-all min-w-[130px] cursor-pointer ${designMode ? 'border-indigo-500 bg-indigo-50/30' : ''}`}>
                        ...
                        <div className="truncate"><h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Layout</h4><p className="text-[10px] font-black text-slate-900 mt-1">{designMode ? 'SAVE LAYOUT' : 'DESIGN MODE'}</p></div>
                    </div>
                    <div onClick={() => { setShowDocs(true); setShowCalc(false); }} className="glass-card p-2.5 flex items-center gap-3 hover:border-indigo-400/50 group transition-all min-w-[130px] cursor-pointer">
                        ...
                        <div className="truncate"><h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Docs</h4><p className="text-[10px] font-black text-slate-900 mt-1">UPLOAD</p></div>
                    </div>
                </div>
                ...
                <div className="relative shrink-0 ml-2" ref={notifRef}>
                    <button onClick={() => setShowNotifs(!showNotifs)} className="w-10 h-10 rounded-xl bg-white border border-slate-100 shadow-sm flex items-center justify-center hover:bg-slate-50 transition-all hover:-translate-y-0.5 relative">
                        ...
                        {unreadCount > 0 && <span className="absolute -top-1 -right-1 min-w-[16px] h-[16px] bg-red-500 rounded-full text-[10px] font-black text-white flex items-center justify-center px-1 border-2 border-white">{unreadCount}</span>}
                    </button>
                    {showNotifs && (
                        <div className="absolute top-[50px] right-0 w-[340px] bg-white border border-slate-100 rounded-2xl shadow-2xl z-[500] overflow-hidden">
                            <div className="p-4 border-b border-slate-50 flex items-center justify-between">
                                <div>
                                    ...
                                    <div className="text-[10px] font-bold text-slate-400">{notifsLoading ? 'Loading…' : `${unreadCount} unread`}</div>
                                </div>
                                {unreadCount > 0 && <button onClick={markAllRead} className="text-[10px] font-black text-indigo-600 uppercase tracking-widest hover:underline">Mark all read</button>}
                            </div>
                            <div className="max-h-[360px] overflow-y-auto custom-scrollbar">
                                {notifsLoading ? (
                                    ...
                                    <div className="flex flex-col items-center justify-center py-10 gap-2">
                                        ...
                                        <span className="text-[10px] font-bold text-slate-400">No notifications</span>
                                    </div>
                                ) : notifList.map(n => (
                                    <div key={n.id} onClick={() => markOneRead(n)} className={`p-3 border-b border-slate-50 flex items-start gap-3 hover:bg-slate-50 cursor-pointer transition-colors ${n.unread ? 'bg-indigo-50/30' : ''}`}>
                            ...
                            <div className="p-3 border-t border-slate-50 text-center">
                                <button onClick={clearAll} className="text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-600">Clear All</button>
                            </div>
                        </div>
                    )}
                </div>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-5 bg-slate-50/30">
                            <div className="w-full bg-white rounded-2xl shadow-sm p-5 border border-slate-100">
                                <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                                    ...
                                    <div className="col-span-2 text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] pt-2 pb-0.5 border-t border-slate-100">Interest Rate Inputs</div>
                                    ...
                                    <div className="col-span-2 text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] pt-2 pb-0.5 border-t border-slate-100">Arrangement Fees</div>
                                ...
                                <div className="mt-6 p-4 bg-slate-50 rounded-xl border border-slate-100">
                                    <div className="text-center mb-4"><span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Estimated Monthly Payment</span><span className="block text-2xl font-black text-emerald-600 mt-1">{fmt(calcResult.monthly)}</span></div>
            ...
            {showDocs && (
                <div className="fixed inset-0 z-[1100] flex justify-end overflow-hidden">
                    <div className="absolute inset-0 bg-slate-900/20 backdrop-blur-[2px]" onClick={() => { setShowDocs(false); setDocView('list'); setSelectedDoc(null); setDocs([]); }} />
                    <div className="relative w-full max-w-[500px] bg-white h-full shadow-[-20px_0_50px_rgba(0,0,0,0.1)] border-l border-slate-100 flex flex-col">
                        ...
                        <div className="px-5 py-3.5 flex items-center justify-between shrink-0" style={{ background: docView === 'detail' && selectedDoc ? (CAT_COLORS[selectedDoc.category] || '#0f172a') : '#0f172a' }}>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <a href="/docs" className="text-[10px] font-black text-indigo-400 bg-indigo-500/15 px-2.5 py-1 rounded-md uppercase tracking-widest hover:bg-indigo-500/25 transition-all">Full View</a>
                                <button onClick={() => { setShowDocs(false); setDocView('list'); setSelectedDoc(null); setDocs([]); }} className="w-7 h-7 rounded-full flex items-center justify-center text-white hover:bg-white/10 transition-all"><i className="fa-solid fa-xmark text-xs"></i></button>
                            </div>
                        </div>
                            ...
                            <div className="px-4 py-2.5 border-b border-slate-100 flex gap-2 items-center shrink-0 bg-slate-50">
                                <div className="relative flex-1">
                                    <input type="text" value={docSearch} onChange={e => setDocSearch(e.target.value)} placeholder="Search documents..."
                                        className="w-full bg-white border border-slate-200 rounded-lg pl-7 pr-3 py-1.5 text-xs font-semibold text-slate-800 outline-none focus:border-indigo-400" />
                                    <i className="fa-solid fa-magnifying-glass absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 text-[10px]"></i>
                                </div>
                                <select value={docCat} onChange={e => setDocCat(e.target.value)} className="bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-bold text-slate-600 outline-none cursor-pointer">
                            ...
                            {docView === 'list' ? (
                                ...
                                docsLoading ? (
                                    <div className="flex flex-col items-center justify-center h-full text-center px-8">
                                        <i className="fa-solid fa-circle-notch fa-spin text-indigo-400 text-2xl mb-3"></i>
                                        <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Loading Documents...</div>
                                    </div>
                                    ...
                                    const fi = FILE_ICONS[d.fileType] || FILE_ICONS.default;
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="text-[10px] text-slate-500 mt-0.5 truncate">{d.desc}</div>
                                                <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                                                    <span className="text-[10px] font-black px-1.5 py-0.5 rounded-full" style={{ background: cc + '22', color: cc }}>{d.category}</span>
                                                    <span className="text-[10px] font-bold text-slate-400">{d.version}</span>
                                                    <span className="text-[10px] font-bold text-slate-400">{d.fileSize}</span>
                                                    <span className="text-[10px] font-bold text-slate-400 ml-auto">{date}</span>
                                                </div>
                                            </div>
                                        </div>
                                    );
                                ...
                                selectedDoc && (
                                    <div className="flex flex-col h-full">
                                        ...
                                        <div className="grid grid-cols-4 gap-0 border-b border-slate-100 shrink-0">
                                            ...
                                            <div className="p-2 border-r border-slate-100">
                                                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Size</div>
                                                <div className="text-xs font-black text-slate-900">{selectedDoc.fileSize}</div>
                                            </div>
                                            <div className="p-2 border-r border-slate-100">
                                                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Version</div>
                                                <div className="text-xs font-black text-slate-900">{selectedDoc.version}</div>
                                            </div>
                                            <div className="p-2">
                                                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Uploaded</div>
                                                <div className="text-xs font-black text-slate-900">{formatDate(selectedDoc.uploaded)}</div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-slate-100 p-5 flex-1">
                                            <div className="bg-white rounded-lg shadow-lg p-8 min-h-[400px] flex flex-col">
                                                ...
                                                <div className="text-center mb-6">
                                                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Alpha Funding CRM</div>
                                        ...
                                        <div className="p-4 border-t border-slate-100 bg-slate-50 flex gap-2 shrink-0">
                                            <button
                                                ...
                                                className="flex-1 flex items-center justify-center gap-2 py-2 bg-slate-100 border border-slate-200 rounded-lg text-[10px] font-black text-slate-600 uppercase tracking-widest hover:bg-slate-200 transition-all"
                                            >
                                            ...
                                            <button
                                                onClick={() => console.log('Downloading:', selectedDoc.title)}
                                                className="flex-1 flex items-center justify-center gap-2 py-2 bg-slate-900 rounded-lg text-[10px] font-black text-white uppercase tracking-widest hover:bg-black transition-all"
                                            >
