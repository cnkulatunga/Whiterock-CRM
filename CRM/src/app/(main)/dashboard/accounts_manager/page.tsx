...
import FinanceCentreCard from '@/components/FinanceCentreCard';
...
export default function AccountsManagerDashboard() {
    ...
    const { cardOrder, saveLayout, resetLayout } = useDashboardLayout('accounts_manager');
    ...
    const renderCard = (key: string) => {
        switch (key) {
            case 'my_leads_pipeline': return canSeeCard(key) ? <TeleAgentLeadsCard title="My Leads" /> : null;
            case 'finance_center': return canSeeCard(key) ? <FinanceCentreCard /> : null;
            case 'pipeline_snapshot': return canSeeCard(key) ? <PipelineSnapshotCard /> : null;
            case 'team_directory': return canSeeCard(key) ? <TeamDirectoryCard /> : null;
            case 'pending_payouts': return canSeeCard(key) ? <RecentActivityCard /> : null;
